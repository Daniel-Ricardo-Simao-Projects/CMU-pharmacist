import 'package:flutter/material.dart';

const Color backgroundColor = Color(0xFFE1F2F3);
const Color primaryColor = Color(0xFF2E7C87);
const Color primaryBorderColor = Color(0xFF112E3C);
const Color accentColor = Color(0xFF143646);
const Color text1Color = Color(0xFF16373C);
const Color text2Color = Color(0xFFFFFFFF);
const Color subtext1Color = Color(0xFF2F2F2F);
Color shadow1Color = Colors.grey.withOpacity(0.2);
LinearGradient glossyColor = LinearGradient(
    begin: Alignment.centerLeft,
    end: Alignment.centerRight,
    colors: [Colors.white.withOpacity(0.2), Colors.white.withOpacity(0.1)]);
const Color textColor = Color(0xFF16373C); // TODO: Remove this
